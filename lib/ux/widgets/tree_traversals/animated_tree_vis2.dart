...
class AnimatedTreeVisualizer2 extends StatefulWidget {
  final TreeNode root; // ✅ Non-nullable root
  final bool isBFS; // ✅ Traversal type (DFS or BFS)

  const AnimatedTreeVisualizer2({
    required super.key,
    required this.root,
    this.isBFS = false, // Default to DFS
  });

  @override
  AnimatedTreeVisualizer2State createState() => AnimatedTreeVisualizer2State();
}

class AnimatedTreeVisualizer2State extends State<AnimatedTreeVisualizer2>
  ...
  // BFS Animation Function
  Future<void> _animateBFS(TreeNode root) async {
  ...
  Widget _buildNode(TreeNode node) {
    return ValueListenableBuilder<NodeState>(
      valueListenable: node.state, // Listen to the node's state
        ...
        return AnimatedPositioned(
          ...
          child: GestureDetector(
            onTap: () {
              ...
              if (widget.isBFS) {
                _animateBFS(node); // Start BFS from this node
              } else {
